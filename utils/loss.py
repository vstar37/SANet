...
class PixLoss(nn.Module):
    ...
    def __init__(self):
        ...
        if 'ual' in self.lambdas_pix_last and self.lambdas_pix_last['ual']:
            self.criterions_last['ual'] = UalLoss()
...
class UalLoss(torch.nn.Module):
    def __init__(self):
        super(UalLoss, self).__init__()

    def forward(self, predictions, gts):
        intermediate = torch.abs(2 * predictions.sigmoid() - 1)
        intermediate_squared = intermediate ** 2
        loss = 1 - intermediate_squared
